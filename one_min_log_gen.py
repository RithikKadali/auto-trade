...
buy_sell=None
os.makedirs("one_min_logs_exit", exist_ok=True)
...
def monitor_market():
    ...
    log_file = f"one_min_logs_exit/{date_str}_full_market_analysis_log.csv" 
...
#        if ((all(buy_conditions) or all(sell_conditions)) and prevRecommendation == "HOLD / AVOID"):
    ...
    date_str = datetime.now().strftime('%Y-%m-%d')
    log_file_2 = f"one_min_logs_exit/{date_str}_market_analysis_log_2_by_prasanna.csv"
...
##############################################################
    date_str = datetime.now().strftime('%Y-%m-%d')
    filter_table_file = f"one_min_logs_exit/{date_str}_market_filter_table_entry_log.csv"
